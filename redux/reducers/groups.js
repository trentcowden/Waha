...
export function groups(state = [], action) {
   switch (action.type) {
      case CREATE_GROUP:
         ...
         return [...state, { name: action.groupName, progress: [], language: action.language, imageSource: action.imageSource, bookmark: 1 ***REMOVED***]
      case EDIT_GROUP:
         ...
         return state.map(group => {
            if (group.name === action.groupName) {
               if (group.progress.includes(action.lessonIndex)) {
                  return { ...group, progress: group.progress.filter(id => id !== action.lessonIndex) ***REMOVED***
               ***REMOVED*** else {
                  return { ...group, progress: [...group.progress, action.lessonIndex] ***REMOVED***
               ***REMOVED***
            ***REMOVED***
            return group
         ***REMOVED***)

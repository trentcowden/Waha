import React from 'react';
import { View, Text, TouchableOpacity, StyleSheet, Platform ***REMOVED*** from 'react-native';
import { connect ***REMOVED*** from 'react-redux'
import { scaleMultiplier ***REMOVED*** from '../constants'

function LessonSwipeBackdrop(props) {

   //// RENDER

   // render complete button conditionally since it could be complete or incomplete
   var completeButton = props.isComplete ?
      <TouchableOpacity 
         style={[styles.buttonContainer, { backgroundColor: "#82868D" ***REMOVED***]***REMOVED***
         onPress={props.toggleComplete***REMOVED***
      >
         <View style={styles.iconContainer***REMOVED***>
            <Icon name='cancel' size={20***REMOVED*** color="#FFFFFF" />
         </View>

      </TouchableOpacity> :
      <TouchableOpacity 
         style={[styles.buttonContainer, { backgroundColor: "#60C239", alignItems: props.isRTL ? "flex-end" : "flex-start" ***REMOVED***]***REMOVED***
         onPress={props.toggleComplete***REMOVED***
      >
         <View style={styles.iconContainer***REMOVED***>
            <Icon name='check-filled' size={20***REMOVED*** color="#FFFFFF" />
         </View>
      </TouchableOpacity>

   return (
      <View style={[styles.lessonSwipeBackdropContainer, { flexDirection: props.isRLT ? "row-reverse" : "row" ***REMOVED***]***REMOVED***>
         {completeButton***REMOVED***
         <TouchableOpacity style={[styles.buttonContainer, { backgroundColor: "#2D9CDB", alignItems: props.isRLT ? "flex-start" : "flex-end" ***REMOVED***]***REMOVED***>
            <View style={styles.iconContainer***REMOVED***>
               <Icon name={Platform.OS === 'ios' ? 'share-ios' : 'share-android'***REMOVED*** size={20***REMOVED*** color="#FFFFFF" />
            </View>
         </TouchableOpacity>
      </View>
   )
***REMOVED***

//// STYLES

const styles = StyleSheet.create({
   lessonSwipeBackdropContainer: {
      width: "100%",
      flex: 1
   ***REMOVED***,
   buttonContainer: {
      flex: 1,
      justifyContent: 'center',
   ***REMOVED***,
   iconContainer: {
      width: 50 * scaleMultiplier,
      justifyContent: "center",
      alignItems: 'center',
   ***REMOVED***,
   label: {
      color: "#FFFFFF",
      fontFamily: 'light',
      fontSize: 10 * scaleMultiplier,
      margin: 2
   ***REMOVED***
***REMOVED***)

function mapStateToProps(state) {
   var activeGroup = state.groups.filter(item => item.name === state.activeGroup)[0];
   return {
      isRTL: state.database[activeGroup.language].isRTL,
   ***REMOVED***
***REMOVED***;

export default connect(mapStateToProps)(LessonSwipeBackdrop);
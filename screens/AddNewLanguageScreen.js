...
function AddNewLanguageScreen (props) {
  ...
  function renderLanguageInstanceItem (languageInstanceList) {
    return (
      ...
      >
        <Text
          style={[
            styles.languageInstanceText,
            { fontFamily: props.font + '-regular' ***REMOVED***
          ]***REMOVED***
        >
          {languageInstanceList.item.displayName***REMOVED***
        </Text>
  ...
  ***REMOVED***
  ...
  var languageListComponent =
    languageInstanceList.filter(
      item => !installedLanguageInstances.includes(item.id)
    ).length != 0 ? (
      <FlatList
        data={languageInstanceList.filter(
          item => !installedLanguageInstances.includes(item.id)
        )***REMOVED***
      ...
      <Text
        style={[
          styles.noMoreLanguagesText,
          { fontFamily: props.font + '-regular' ***REMOVED***
        ]***REMOVED***
      >
        {props.translations.labels.noMoreLanguages***REMOVED***
      </Text>
    )

  return <View style={styles.screen***REMOVED***>{languageListComponent***REMOVED***</View>
***REMOVED***
